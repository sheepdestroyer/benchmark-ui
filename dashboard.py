    ...
    if not filtered_df.empty:
        ...
        with col_plot2:
            ...
            if not filtered_df.empty:
                ...
                kv_quant_idx = filtered_df.columns.get_loc('KV Quant')
                test_idxs = {test: filtered_df.columns.get_loc(test) for test in TEST_SUITES if test in filtered_df.columns}

                for row in filtered_df.itertuples(index=False, name=None):
                    model_val = row[model_idx]
                    kv_quant_val = row[kv_quant_idx]
                    for test, idx in test_idxs.items():
                        val = row[idx]
                        if val in PASS_FAIL_STATUSES:
                            acc_data.append({
                                "Model_Quant": f"{model_val} ({kv_quant_val})",
                                "Test Suite": test,
