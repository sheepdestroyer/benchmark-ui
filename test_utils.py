...
def test_validate_endpoint_url_invalid_ports():
    # Non-numeric ports
    for url in ["http://example.com:abc", "http://localhost:abc", "https://example.com:80a"]:
    ...
    # Ports > 65535
    for url in ["http://example.com:65536", "http://localhost:65536", "http://example.com:99999"]:
        with pytest.raises(ValueError, match="Invalid port"):
            validate_endpoint_url(url)
...
def test_validate_endpoint_url_dns_resolution_failures():
    ...
    with patch("socket.getaddrinfo", side_effect=socket.gaierror(socket.EAI_NONAME, "Name or service not known")):
        with pytest.raises(ValueError, match="Could not resolve hostname: mock-gaierror.test"):
            validate_endpoint_url("http://mock-gaierror.test")

    # Mock socket.herror
    with patch("socket.getaddrinfo", side_effect=socket.herror(1, "Unknown host")):
        with pytest.raises(ValueError, match="Could not resolve hostname: mock-herror.test"):
...
def test_validate_endpoint_url_multi_ip_resolution_blocking():
    ...
    with patch("socket.getaddrinfo", return_value=addrs_private):
        with pytest.raises(ValueError, match=r"Forbidden IP address range: multi-ip\.test \(10\.0\.0\.1\)"):
    ...
    with patch("socket.getaddrinfo", return_value=addrs_loopback):
        with pytest.raises(ValueError, match=r"Forbidden IP address range: multi-ip\.test \(127\.0\.0\.1\)"):
            validate_endpoint_url("http://multi-ip.test")
    ...
    with patch("socket.getaddrinfo", return_value=addrs_public):
        assert validate_endpoint_url("http://multi-public.example.com") == "http://multi-public.example.com"
...
def test_validate_endpoint_url_ipv6_loopback_and_private():
    ...
    with pytest.raises(ValueError, match=r"Forbidden IP address range: fd12:3456:789a::1"):
    ...
    with patch("socket.getaddrinfo", return_value=addrs_ipv6_global):
        assert validate_endpoint_url("http://[2606:4700:4700::1111]:80") == "http://[2606:4700:4700::1111]:80"
...
def test_validate_endpoint_url_non_ip_addrinfo_fallback():
    ...
    addrs_invalid_ip = [
        (socket.AF_UNSPEC, 0, 0, '', ('non-ip-address-string', 0))
    ]
    with patch("socket.getaddrinfo", return_value=addrs_invalid_ip):
        assert validate_endpoint_url("http://custom-service.domain.test") == "http://custom-service.domain.test"


def test_validate_model_name():
    assert validate_model_name("llama-3:8b") == "llama-3:8b"
    assert validate_model_name("meta-llama/Llama-2-7b-chat-hf") == "meta-llama/Llama-2-7b-chat-hf"
    assert validate_model_name("qwen2.5-coder:32b") == "qwen2.5-coder:32b"
    assert validate_model_name("model_v1.0") == "model_v1.0"

    for invalid in ["", None, "invalid model name;", "model rm -rf", "model$name", "model\nname"]:
